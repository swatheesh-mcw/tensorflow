...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (output->type) {
    case kTfLiteFloat32: {
      optimized_ops::Round<float>(GetTensorShape(input), GetTensorData<float>(input),
                      GetTensorShape(output), GetTensorData<float>(output));
      break;
    }
    case kTfLiteFloat16: {
      optimized_ops::Round<Eigen::half>(GetTensorShape(input), GetTensorData<Eigen::half>(input),
                      GetTensorShape(output), GetTensorData<Eigen::half>(output));
      break;
    }
    case kTfLiteBFloat16: {
      optimized_ops::Round<Eigen::bfloat16>(GetTensorShape(input), GetTensorData<Eigen::bfloat16>(input),
